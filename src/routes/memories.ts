...
export const memoriesRoutes = async (app: FastifyInstance): Promise<void> => {
  app.get('/memories', async (request: FastifyRequest, response: FastifyReply) => {
    const memories = await prisma.memory.findMany({
      orderBy: {
        createdAt: 'asc'
      }
    })
    return await response.status(200).send(memories.map(memory => {
      return {
        id: memory.id,
  ...
  app.get('/memories/:id', async (request: FastifyRequest, response: FastifyReply) => {
  ...
  app.post('/memories', async (request: FastifyRequest, response: FastifyReply) => {
    const { content, coverUrl, isPublic } = createMemoryValidation.parse(request.body)
  ...
  app.put('/memories/:id', async (request: FastifyRequest, response: FastifyReply) => {
    const { id } = memoriesParamsValidation.parse(request.params)
    const { content, coverUrl, isPublic } = updateMemoryValidation.parse(request.body)
  ...
  app.delete('/memories/:id', async (request: FastifyRequest, response: FastifyReply) => {
    const { id } = memoriesParamsValidation.parse(request.params)
